...
// Added by Mid uwu
  const manager = new GiveawaysManager(client, {
    storage: './database/giveaways.json', // any
    updateCountdownEvery: parseInt(options.giveawayCountdown) || 30000,
    default: {
        botsCanWin: false, 
        embedColor: '#2ecc71', //green
        embedColorEnd: '#e74c3c', // red
        reaction: '🎉' // if u want a custom discord emoji use its id
    }
})
    ...
    for (const { name } of dirents) {
      ...
      for (const cmd of cmds) {
        ...
        try {
          ...
          debugs.push(
            `| ❤️ Failed '${cmd.name || cmd.channel}' (${cmd.type})`
          );
